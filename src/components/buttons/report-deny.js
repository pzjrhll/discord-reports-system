...
module.exports = {
	...
	async execute(interaction, client, actionId) {
		...
		await client.logAction(config.logsDenyWebhook, `Administrator **ODRZUCIŁ** zgłoszenie o ID \`${actionId}\``, interaction, null, true);
		return await client.logAction(config.logsAllWebhook, `Administrator **ODRZUCIŁ** zgłoszenie o ID \`${actionId}\``, interaction, null, true);
	},
};
